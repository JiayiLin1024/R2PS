...
import json
import numpy as np
from model import CodeBERT, CodeBERT_RR
...
def evaluate(args, mono_model, sia_model, tokenizer, file_name,codebase_file):
    ...
    for i in range(0, rerank_code_ids.shape[0], args.eval_batch_size):
        ...
        with torch.no_grad():
            ...
            scores[rerank_query_ids[i:i+args.eval_batch_size], rerank_code_ids[i:i+args.eval_batch_size]] = \
                args.alpha * scores[rerank_query_ids[i:i+args.eval_batch_size], rerank_code_ids[i:i+args.eval_batch_size]] + (1-args.alpha) * code_nl_score.reshape(-1) + 100
...
def main():
    ...
    parser.add_argument("--output_dir_sia_model", default="saved_model/codebert/CSN/ruby/", type=str, 
                        help="The saved model checkpoint.")
    parser.add_argument("--output_dir_mono_model", default="saved_model/codebert_rr/CSN/ruby/", type=str, 
                        help="The saved model checkpoint.")
    ...
    sia_model = CodeBERT(RobertaModel.from_pretrained(args.model_name_or_path))
    mono_model = CodeBERT_RR(RobertaModel.from_pretrained(args.model_name_or_path))
