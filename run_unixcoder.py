...
def train(args, model, tokenizer):
    ...
    for idx in range(args.num_train_epochs): 
        for step,batch in enumerate(train_dataloader):
            ...
            loss = loss_fct(scores * args.temperature, torch.arange(code_inputs.size(0), device=scores.device))
...
def main():
    ...
    parser.add_argument("--output_dir", default="saved_model/unixcoder/CSN/ruby/", type=str, 
    ...
    parser.add_argument("--log", default="log/log_0.log", type=str,
                        help="log_file.")
    ...
    model_to_load.load_state_dict(torch.load(os.path.join(args.output_dir, "model.bin")))
    eval_data_file, codebase_file = args.eval_data_file, args.codebase_file
    result = evaluate(args, model, tokenizer, eval_data_file, codebase_file)
    logger.info("***** Eval results on validation set *****")
    for key in sorted(result.keys()):
